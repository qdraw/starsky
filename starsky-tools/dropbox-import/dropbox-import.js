...
if (argsArray.indexOf("-h") >= 0 || argsArray.indexOf("--help") >= 0) {
	console.log(
		"Dropbox Import Help\n1. update .env file \n" +
			"2. add as arg the folder in the dropbox, default = '" +
			dropboxFolder +
			"'"
	);
...
for (var arg = 0; arg < argsArray.length; arg++) {
	if (
		argsArray[arg].toLowerCase() != "--colorclass" ||
		arg + 1 == argsArray.length
	)
		continue;
...
for (var arg = 0; arg < argsArray.length; arg++) {
	if (
		(argsArray[arg].toLowerCase() == "--path" ||
			argsArray[arg].toLowerCase() == "-p") &&
		arg + 1 != argsArray.length
	) {
...
for (var arg = 0; arg < argsArray.length; arg++) {
	if (
		argsArray[arg].toLowerCase() != "--structure" ||
		arg + 1 == argsArray.length
	)
		continue;
...
if (colorClassString)
	console.log(
		">> using the --colorclass value: " + "'" + colorClassString + "'"
	);
if (structure)
	console.log(">> using the --structure value: " + "'" + structure + "'");

var dropboxCore = require("./dropbox-core");

(async () => {
	// get tokens
	const accessToken = await require("./dropbox-get-access-token")(
		process.env.DROPBOX_CLIENT_ID,
		process.env.DROPBOX_CLIENT_SECRET,
		process.env.DROPBOX_REFRESH_TOKEN
	);

	const dropbox = new dropboxCore(
		accessToken,
		process.env.STARSKYIMPORTERCLI
	);
				...
				dropbox
					...
					.then((entries) => {
						if (process.env.DEBUG === "true")
							console.log("DEBUG MODE == NO DELETE");
