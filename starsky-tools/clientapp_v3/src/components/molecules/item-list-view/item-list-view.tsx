 ...
 */
const ItemListView: React.FunctionComponent<ItemListProps> = memo(
	({ fileIndexItems, ...props }) => {
		...
		if (!fileIndexItems) return <div className="folder">no content</div>;

		return (
			<div className="folder" ref={folderRef}>
				{props.pageType !== PageType.Loading ? (
					fileIndexItems.length === 0 ? (
						props.colorClassUsage.length >= 1 ? (
							<div className="warning-box warning-box--left">{MessageItemsOutsideFilter}</div>
						) : (
