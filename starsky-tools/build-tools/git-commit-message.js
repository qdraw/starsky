...
const getPreviousGitUserEmail = spawnSync('git', ['config', '--global', 'user.email'], {
    cwd: searchPath,
    env: process.env,
    encoding: 'utf-8'
});

const getPreviousGitUserName = spawnSync('git', ['config', '--global', 'user.name'], {
    cwd: searchPath,
    env: process.env,
    encoding: 'utf-8'
});

const setGitUserEmail = spawnSync('git', ['config', '--global', 'user.email', gitUserEmail], {
    cwd: searchPath,
    env: process.env,
    encoding: 'utf-8'
});

const setGitUserName = spawnSync('git', ['config', '--global', 'user.name', gitUserName], {
    cwd: searchPath,
    env: process.env,
    encoding: 'utf-8'
});

console.log(`setGitUserEmail: ${gitUserEmail} -  ${setGitUserEmail.stdout} \n ${setGitUserEmail.stderr}`);
console.log(`setGitUserName: ${gitUserName} -  ${setGitUserName.stdout}\n ${setGitUserName.stderr}`);
...
// and put it back
const setGitUserEmailBack = spawnSync('git', ['config', '--global', 'user.email', getPreviousGitUserEmail.stdout], {
    cwd: searchPath,
    env: process.env,
    encoding: 'utf-8'
});

const setGitUserNameBack = spawnSync('git', ['config', '--global', 'user.name', getPreviousGitUserName.stdout], {
    cwd: searchPath,
    env: process.env,
    encoding: 'utf-8'
});
