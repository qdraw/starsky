	...
	connection
		...
		.then(function (attachments) {
			...
			for (var i = 0; i < attachments.length; i++) {
				...
				console.log(
					"file: " + filename + " (" + attachments[i].filename + ")"
				);
				...
				request(
					{
						headers: {
							Authorization:
								"Basic " + process.env.STARKSYACCESSTOKEN,
							Structure:
								"/yyyy/MM/yyyy_MM_dd*/__\\g\\p\\x__yyyyMMdd_HHmmss_{filenamebase}.ext",
						},
						formData: formData,
					...
					function (err, res, body) {
						...
						if (
							document[0].fileIndexItem &&
							document[0].fileIndexItem.parentDirectory
						) {
							updateGeoData(
								document[0].fileIndexItem.parentDirectory
							);
	...
	request(
		{
			...
			formData: formData,
			uri: process.env.STARKSYGEOURL,
			method: "POST",
		},
		function (err, res, body) {
			console.log(
				"GEO " + parentDirectory + " status: " + res.statusCode
			);
		}
	);
}
