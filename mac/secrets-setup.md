...
In `.github/workflows/desktop-release-on-tag-native.yml`, the certificate import step (from `mac/scripts/import-cert.sh`) must run **before** each `Archive` step in `build_mac_native`, `build_mac_arm64`, and `build_mac_x64`. Add the keychain cleanup step after each job's `Notarize and staple` step.
