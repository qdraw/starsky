...
To enable debug mode, set `app__verbose` to `true` in the `environment:` section
...
*Note that high-resolution panoramic images may require additional swap space and/or [physical memory](performance.md#memory) above the [recommended minimum](../readme.mdx#system-requirements).*
...
Windows Pro users should [disable](../../assets/getting-started-docker-disable-wsl2.jpg) the *WSL 2* based engine in *Docker Settings > General*
...
It is important to explicitly [increase the Docker memory limit](../../assets/getting-started-docker-resources-advanced.jpg) to 4 GB or more
...
It is important to [increase the Docker memory limit](../../assets/getting-started-docker-resources-advanced.jpg) to 4 GB or more, as the
...
### Note
   **Be very careful when changing permissions in shared hosting environments.** If you are using Starsky on corporate
...
- if you are using *Kubernetes*, *Docker Desktop*, *Hyper-V*, or a Virtual Machine, they have their own settings to adjust the size of [storage](../docker-compose.md#volumes), [RAM](../readme.md#system-requirements), and [swap](#adding-swap)
