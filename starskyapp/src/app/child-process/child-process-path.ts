...
export function childProcessPath(): string {
    ...
    if (!isPackaged()) { // dev
        switch (process.platform) {
            case "darwin":
                return path.join(__dirname, "../", "../", "../", "../", "starsky", "osx.10.12-x64", "starsky");
            case "win32":
                return path.join(__dirname, "../", "../", "../", "../", "starsky", "win7-x64", "starsky.exe");
            default:
                throw new Error("not implemented")
        }
    }

    var targetFilePath = path.join(process.resourcesPath, `runtime-starsky-${OsBuildKey()}`);
