...
public sealed class StatusCodesHelper(AppSettings appSettings)
{
	public FileIndexItem.ExifStatus IsReadOnlyStatus(FileIndexItem fileIndexItem)
	{
		if ( fileIndexItem.IsDirectory == true &&
		     appSettings.IsReadOnly(fileIndexItem.FilePath!) )
		{
			return FileIndexItem.ExifStatus.DirReadOnly;
		}

		if ( appSettings.IsReadOnly(fileIndexItem.ParentDirectory!) )
		{
	...
	public FileIndexItem.ExifStatus IsReadOnlyStatus(DetailView? detailView)
	{
		...
		if ( detailView.IsDirectory && appSettings.IsReadOnly(detailView.SubPath) )
		{
			return FileIndexItem.ExifStatus.DirReadOnly;
		}

		if ( appSettings.IsReadOnly(detailView.FileIndexItem?.ParentDirectory!) )
		{
			return FileIndexItem.ExifStatus.ReadOnly;
		}
