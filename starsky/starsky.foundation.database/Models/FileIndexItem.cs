...
public sealed class FileIndexItem
{
	...
	///     with the first item.
	/// </summary>
	private readonly List<ImageRotation.Rotation> _orderRotation = new()
	{
		ImageRotation.Rotation.Horizontal,
		ImageRotation.Rotation.Rotate90Cw,
		ImageRotation.Rotation.Rotate180,
		ImageRotation.Rotation.Rotate270Cw
	};
	...
	/// <summary>
	///     Unique database Id, not used and Json Ignored due the fact that files that are moved could have
	///     a new Id
	/// </summary>
	/// <value>
	///     The identifier.
	...
	[JsonConverter(typeof(JsonStringEnumConverter))]
	// newtonsoft uses: StringEnumConverter
	public ImageRotation.Rotation Orientation { get; set; } = ImageRotation.Rotation.DoNotChange;
	...
	public string MakeCameraSerial
	{
		get
		{
			...
			var makeModelList = MakeModel?.Split('|');
			var serial = makeModelList is { Length: < 4 } ? string.Empty : makeModelList![3];
	...
	public ImageRotation.Rotation RelativeOrientation(int relativeRotation = 0)
	{
	...
	/// <returns></returns>
	public ImageRotation.Rotation SetAbsoluteOrientation(string orientationString = "0")
	{
		switch ( orientationString )
		{
			...
			case "3":
				...
				return Orientation;
			case "8":
				Orientation = ImageRotation.Rotation.Rotate270Cw;
				return Orientation;
			default: // Fallback by for example 'null'
				Orientation = ImageRotation.Rotation.DoNotChange;
				return Orientation;
		}
	}
