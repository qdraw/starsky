...
public static class MergeCoverageFiles
{
	...
	static bool FileExists(string value) => File.Exists(value);

	static void DeleteFile(string value) => File.Delete(value);

	static void MoveFile(string from, string to) => File.Move(from, to, true);

	static void CopyFile(string from, string to) => File.Copy(from, to, true);
	...
	public static void Merge(bool noUnitTest)
	{
		...
		var jestCoverageFile = Path.Combine(rootDirectory,
			"starsky/clientapp/coverage/cobertura-coverage.xml");
		...
		var outputCoverageFile =
			Path.Combine(rootDirectory, "starskytest/coverage-merge-cobertura.xml");
		...
		var outputCoverageSonarQubeFile =
			Path.Combine(rootDirectory, "starskytest/coverage-merge-sonarqube.xml");
