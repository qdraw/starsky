	...
	: ICameraStorageDetector
{
	...
	public bool IsCameraStorage(string driveRoot)
	{
		...
		{
			logger.LogError($"Drive root failed: '{driveRoot}' {ex.Message} {ex.StackTrace}", ex);
			return false;
		}
	}
	...
	public bool IsCameraStorage(CameraDriveInfo drive)
	{
		...
		{
			logger.LogError(
				$"No IsCameraFriendlyFileSystem: Path: {drive.RootDirectory.FullName} DriveFormat: {drive.DriveFormat}");
			return false;
		}
