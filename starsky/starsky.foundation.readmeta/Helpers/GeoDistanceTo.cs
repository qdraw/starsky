...
public static class GeoDistanceTo
{
	...
	public static double GetDistance(double latitudeFrom,
		double longitudeFrom, double latitudeTo,
		double longitudeTo)
	{
		var dlon = Radians(longitudeTo - longitudeFrom);
		var dlat = Radians(latitudeTo - latitudeFrom);

		var a = Math.Sin(dlat / 2) * Math.Sin(dlat / 2) +
		        Math.Cos(Radians(latitudeFrom)) * Math.Cos(Radians(latitudeTo)) *
