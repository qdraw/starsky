...
namespace starskytest.Controllers
{
	...
	public sealed class AccountControllerTest
	{
		...
		public async Task AccountController_NoLogin_Login_And_newAccount_Test()
		{
			...
			// And clean afterward
			var itemWithId = await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == newAccount.Name, TestContext.CancellationToken);
		...
		{
			AccountController controller = new AccountController(new FakeUserManagerActiveUsers(),
		...
		public async Task AccountController_Model_is_not_correct_NoUsersActive()
		{
			...
			{
				Email = "test", ConfirmPassword = "1", Password = "2", Name = "NoUsersActive"
			};
		...
		public async Task AccountController_Model_WithUsersActive_GetRegisterPage_Forbid()
		{
			...
			var registerViewModel = new RegisterViewModel
			{
				Email = "test", ConfirmPassword = "1", Password = "2", Name = "Forbid"
			};
		...
		public async Task AccountController_ChangeSecret_NotLoggedIn()
		{
			...
			var actionResult =
				await controller.ChangeSecret(changePasswordViewModel) as UnauthorizedObjectResult;
		...
		public async Task AccountController_ChangeSecret_WrongInput()
		{
			...
			{
				Password = "oldPassword",
				ChangedPassword = "newPassword1111",
				ChangedConfirmPassword = "newPassword"
			};

			var actionResult =
				await controller.ChangeSecret(changePasswordViewModel) as BadRequestObjectResult;
		...
		public async Task AccountController_Model_WithUsersActive_GetRegisterPage_BlockedByDefault()
		{
			...
			var registerViewModel = new RegisterViewModel
			{
				Email = "test", ConfirmPassword = "1", Password = "2", Name = "blockedByDefault"
			};
		...
		public async Task AccountController_LoginContext_GetRegisterPage_AccountCreated()
		{
			...
			var getUser = await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == user.Name, TestContext.CancellationToken);
		...
		{
			var controller = new AccountController(
				new FakeIUserManger(new UserOverviewModel(new List<User> { new User() })),
				_appSettings, _antiForgery, _selectorStorage)
			{
				ControllerContext =
				{
		...
		public async Task AccountController_newAccount_TryToOverwrite_ButItKeepsTheSamePassword()
		{
			...
			var login = new LoginViewModel { Email = userId, Password = "test" };

			var newAccount = new RegisterViewModel
			{
				Password = "test", ConfirmPassword = "test", Email = userId, Name = userId
			};
			...
			// Clean afterwards            
			var user = await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == userId, TestContext.CancellationToken);
		...
		public async Task AccountController_LoginStatus_NoAccounts()
		{
			...
			Assert.AreEqual("There are no accounts, you must create an account first",
