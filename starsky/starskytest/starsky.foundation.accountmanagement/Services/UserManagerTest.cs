...
public sealed class UserManagerTest
{
	...
	public async Task ValidateAsync_Credential_NotFound()
	{
		...
		var test = await _dbContext.CredentialTypes.AnyAsync(p => p.Code == "email", TestContext.CancellationToken);
		if ( !test )
		{
	...
	public async Task ValidateAsync_User_NotFound()
	{
		...
		var credentialTypesCode = await _dbContext.CredentialTypes.FirstOrDefaultAsync(TestContext.CancellationToken);
	...
	public async Task ValidateAsync_Transform_To_Iterate100K(bool cacheEnabled)
	{
		...
		await _dbContext.Users.AddAsync(new User { Name = "test_0008" }, TestContext.CancellationToken);
		await _dbContext.SaveChangesAsync(TestContext.CancellationToken);
		...
		var cred = new Credential
		{
			...
			User = await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == "test_0008", TestContext.CancellationToken),
			UserId = ( await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == "test_0008", TestContext.CancellationToken) )!.Id,
			Id = 43579345
		};
		var credExists = await _dbContext.Credentials.AnyAsync(p => p.Identifier == "test_0008", TestContext.CancellationToken);
		if ( !credExists )
		{
		...
		var credAfterTransform = await _dbContext.Credentials
			.FirstOrDefaultAsync(p => p.Identifier == "test_0008", TestContext.CancellationToken);
	...
	public async Task ValidateAsync_LockoutEnabled()
	{
		...
		var userObject = await _dbContext.Users.FirstOrDefaultAsync(p =>
			p.Name == "lockout@google.com", TestContext.CancellationToken);
	...
	public async Task ValidateAsync_3thTry()
	{
		...
		var userObject = await _dbContext.Users.FirstOrDefaultAsync(p =>
			p.Name == "try3@google.com", TestContext.CancellationToken);
	...
	public async Task ValidateAsync_ResetCountAfterSuccessLogin()
	{
		...
		var userObject = await _dbContext.Users.FirstOrDefaultAsync(p =>
			p.Name == "reset@google.com", TestContext.CancellationToken);
	...
	public async Task ValidateAsync_LockoutExpired()
	{
		...
		var userObject = await _dbContext.Users.FirstOrDefaultAsync(p =>
			p.Name == "lockout2@google.com", TestContext.CancellationToken);
	...
	public async Task ChangeSecret_Credential_NotFound()
	{
		...
		var emailType =
			await _dbContext.CredentialTypes.FirstOrDefaultAsync(p => p.Code == "email", TestContext.CancellationToken);
	...
	public async Task GetRoleAsync_Exists()
	{
		...
		await _dbContext.SaveChangesAsync(TestContext.CancellationToken);
		var role =
			await _dbContext.Roles.FirstOrDefaultAsync(p => p.Code == "test_role_892453", TestContext.CancellationToken);
		var userRole =
			await _dbContext.UserRoles.FirstOrDefaultAsync(p => p.UserId == 45475, TestContext.CancellationToken);
		var user =
			await _dbContext.Users.FirstOrDefaultAsync(p => p.Id == 45475, TestContext.CancellationToken);
	...
	public async Task CachedCredential_CheckCache()
	{
		...
		await _dbContext.CredentialTypes.AddAsync(credType, TestContext.CancellationToken);
		await _dbContext.SaveChangesAsync(TestContext.CancellationToken);
		...
		var cred =
			await _dbContext.Credentials.FirstOrDefaultAsync(p =>
				p.Identifier == "test_cache_add", TestContext.CancellationToken);
	...
	public async Task UserManager_GetRoleAddToUser_Administrator()
	{
		...
		await _dbContext.Users.AddAsync(beforeItem, TestContext.CancellationToken);
		await _dbContext.SaveChangesAsync(TestContext.CancellationToken);
...
#pragma warning restore CS8670 // Object or collection initializer implicitly dereferences possibly null member.
	...
	public async Task UserManager_GetRoleAddToUser_IgnoreItself()
	{
		...
		foreach ( var user in await _dbContext.Users.ToListAsync(TestContext.CancellationToken) )
		{
		...
		await _dbContext.Users.AddAsync(new User { Name = id }, TestContext.CancellationToken);
		await _dbContext.SaveChangesAsync(TestContext.CancellationToken);
		var beforeItem =
			await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == id, TestContext.CancellationToken);

		Assert.IsNotNull(beforeItem);
		Assert.AreEqual(id,
			( await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == id, TestContext.CancellationToken) )?.Name);
		...
		// clean user
		var item = await _dbContext.Users.FirstOrDefaultAsync(p => p.Name == id, TestContext.CancellationToken);
