...
public sealed class MetaUpdateServiceTest
{
	...
	public MetaUpdateServiceTest()
	{
		...
		_query = new Query(dbContext, new AppSettings(), null!, new FakeIWebLogger(),
			_memoryCache);
	...
	public async Task UpdateService_Update_toDelete()
	{
		...
		var item0 = await query.GetObjectByFilePathAsync("/delete/test_delete.jpg");
		...
		Assert.AreEqual(TrashKeyword.TrashKeywordString,
			( await query.GetObjectByFilePathAsync("/delete/test_delete.jpg") )!.Tags);

		Assert.AreEqual("noChanges",
			( await query.GetObjectByFilePathAsync("/delete/test_delete.jpg") )!.Description);
	...
	public async Task Update_Write_GPX()
	{
		...
		{
			Tags = "test", Status = FileIndexItem.ExifStatus.Ok
		};
	...
	{
		var detailView = new DetailView { FileIndexItem = new FileIndexItem("/test.jpg") };
