...
[TestClass]
public class ImageOptimisationChmodTests
{
	private readonly FfmpegExePath _ffmpegExePath;
	private readonly StorageHostFullPathFilesystem _hostFileSystemStorage;
	private readonly ImageOptimisationChmod _imageOptimisationChmod;
	private readonly bool _isWindows;
	private readonly IWebLogger _logger;
	private readonly string _parentFolder;

	public ImageOptimisationChmodTests()
	{
		_hostFileSystemStorage = new StorageHostFullPathFilesystem(new FakeIWebLogger());
		_logger = new FakeIWebLogger();
		_imageOptimisationChmod =
			new ImageOptimisationChmod(new FakeSelectorStorage(_hostFileSystemStorage), _logger);

		_parentFolder = Path.Combine(new CreateAnImage().BasePath, "ImageOptimisationChmodTests");
	...
	{
		var sut = new ImageOptimisationChmod(new FakeSelectorStorage(), new FakeIWebLogger());
	...
	public async Task Chmod_ShouldReturnTrue_WhenCommandSucceeds__UnixOnly()
	{
		...
		var result = await _imageOptimisationChmod.Chmod(_ffmpegExePath.GetExePath("linux-x64"));
	...
	public async Task Chmod_ShouldReturnFalse_WhenCommandFails__UnixOnly()
	{
		...
		var sut = new ImageOptimisationChmod(new FakeSelectorStorage(new FakeIStorage([],
				["/bin/chmod"])),
			new FakeIWebLogger());
	...
	public async Task Chmod_ShouldLogError_WhenCommandFails__UnixOnly()
	{
		...
		var result = await _imageOptimisationChmod.Chmod("/_not_found_path/to/ffmpeg");
		Assert.IsFalse(result);
