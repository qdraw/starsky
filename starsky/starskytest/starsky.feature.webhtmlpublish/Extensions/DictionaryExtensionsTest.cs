...
public sealed class DictionaryExtensionsTest
{
	...
	{
		var dictionary = new Dictionary<string, bool> { { "t", true } };

		dictionary.AddRangeOverride(new Dictionary<string, bool> { { "t", false } });
	...
	public void ForEach()
	{
		...
		dictionary.ForEach(p => { Assert.AreEqual("t", p.Key); });
	}
}
