...
public class TestForNonAuthorizeAttributesTest
{
    ...
    {
        var getEverythingBeforeLastDotRegex = new Regex(".*(?=\\.)", RegexOptions.None, TimeSpan.FromMilliseconds(100));
    ...
    private static List<MethodInfo> GetControllerMethods(Type projectController)
    {
        ...
        var controllersBaseInNamespace = GetControllersInNamespace(controllerBaseAssembly, controllerBaseName);
        var controllersBaseInNamespaceMethods = controllersBaseInNamespace.SelectMany(p => p.GetMethods()).ToArray();
        
        var (controllerGenericTypeAssembly, controllerGenericTypeName) = GetAssembly<Controller>();
        var controllersGenericTypeInNamespace = GetControllersInNamespace(controllerGenericTypeAssembly, controllerGenericTypeName);
        var controllersGenericTypeNamespaceMethods = controllersGenericTypeInNamespace.SelectMany(p => p.GetMethods()).ToArray();
        
        var allGenericControllerTypes = controllersBaseInNamespaceMethods.Concat(controllersGenericTypeNamespaceMethods).ToArray();
    ...
    public void TestForNonAuthorizeAttributes()
    {
        ...
        foreach (var controller in controllersInNamespace)
        {
            ...
            foreach (var method in methods)
            {
                ...
                var authorizeParentAttributes = method.DeclaringType?.GetCustomAttributes(typeof(AuthorizeAttribute), true) ?? Array.Empty<object>();
                var allowAnonymousAttributes = method.GetCustomAttributes(typeof(AllowAnonymousAttribute), true);
                var allowAnonymousParentAttributes = method.DeclaringType?.GetCustomAttributes(typeof(AllowAnonymousAttribute), true) ?? Array.Empty<object>();
