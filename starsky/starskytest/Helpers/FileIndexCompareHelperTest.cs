...
public sealed class FileIndexCompareHelperTest
{
	...
	[TestMethod]
	public void FileIndexCompareHelper_AiFields_Compare()
	{
		var source = new FileIndexItem
		{
			SuggestedTags = "cat",
			RejectedTags = "",
			ImageClassificationModel = "v1",
			ImageClassificationGeneratedAt = new DateTime(2025, 1, 1, 0, 0, 0, DateTimeKind.Utc)
		};
		var update = new FileIndexItem
		{
			SuggestedTags = "cat, park",
			RejectedTags = "car",
			ImageClassificationModel = "v2",
			ImageClassificationGeneratedAt =
				new DateTime(2026, 4, 21, 10, 20, 30, DateTimeKind.Utc)
		};

		var changed = FileIndexCompareHelper.Compare(source, update);

		Assert.AreEqual("cat, park", source.SuggestedTags);
		Assert.AreEqual("car", source.RejectedTags);
		Assert.AreEqual("v2", source.ImageClassificationModel);
		Assert.AreEqual(new DateTime(2026, 4, 21, 10, 20, 30, DateTimeKind.Utc),
			source.ImageClassificationGeneratedAt);

		Assert.Contains(nameof(FileIndexItem.SuggestedTags).ToLowerInvariant(), changed);
		Assert.Contains(nameof(FileIndexItem.RejectedTags).ToLowerInvariant(), changed);
		Assert.Contains(nameof(FileIndexItem.ImageClassificationModel).ToLowerInvariant(), changed);
		Assert.Contains(nameof(FileIndexItem.ImageClassificationGeneratedAt).ToLowerInvariant(),
			changed);
	}
}
