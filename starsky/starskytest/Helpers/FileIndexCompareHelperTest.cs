...
public sealed class FileIndexCompareHelperTest
{
	...
	{
		var source = new FileIndexItem { Orientation = RotationModel.Rotation.Horizontal };
		var update = new FileIndexItem { Orientation = RotationModel.Rotation.Rotate90Cw };
		FileIndexCompareHelper.Compare(source, update);
		Assert.AreEqual(RotationModel.Rotation.Rotate90Cw, source.Orientation);
	}
	...
	public void CompareRotation_NotFound()
	{
		...
		FileIndexCompareHelper.CompareRotation("t",
			...
			RotationModel.Rotation.Horizontal, list);
	...
	public void TestCompare_NullRotation()
	{
		...
		var sourceIndexItem = new FileIndexItem();
		var updateObject = new FileIndexItem();
		sourceIndexItem.Orientation = RotationModel.Rotation.DoNotChange;
