...
public sealed class PathHelper2Test
{
	...
	{
		var input =
			PathHelper.RemoveLatestBackslash("/2018" + Path.DirectorySeparatorChar);
		var output = "/2018";
	...
	public void ConfigRead_AddBackslashTest()
	{
		...
		var output = "2018" + Path.DirectorySeparatorChar;
		Assert.AreEqual(input, output);

		input = PathHelper.AddBackslash("2018" + Path.DirectorySeparatorChar);
		output = "2018" + Path.DirectorySeparatorChar;
		Assert.AreEqual(input, output);
	}
