...
namespace starskytest.starsky.foundation.search.Services
{
	...
	public sealed class SearchServiceTest
	{
		...
		public async Task SearchService_SearchForDatetime()
		{
			await InsertSearchData();
			Assert.AreEqual(1,
				( await _search.Search("-Datetime=\"2016-01-01 01:01:01\"") ).SearchCount);
		}
		...
		public async Task SearchService_SearchForDatetimeSmallerThen()
		{
			await InsertSearchData();
			Assert.AreEqual(2,
				( await _search.Search("-Datetime<\"2013-01-01 02:01:01\"") ).SearchCount);
		}
		...
		public async Task SearchService_SearchForLastEdited()
		{
			await InsertSearchData();
			Assert.AreEqual(1,
				( await _search.Search("-LastEdited=\"2020-10-10 10:10:10\"") ).SearchCount);
		}
		...
		{
			await InsertSearchData();
			Assert.AreEqual(NumberOfFakeResults + 1,
				( await _search.Search("-Datetime>\"2018-01-01 01:01:01\"") ).SearchCount);
		}
		...
		public async Task SearchService_SearchSchipholFilenameTest()
		{
			await InsertSearchData();
			Assert.AreEqual(1,
				( await _search.Search("-filename:'schipholairplane.jpg'") ).SearchCount);
		}
		...
		{
			await InsertSearchData();
			Assert.AreEqual(NumberOfFakeResultsThatFitOnPage,
				( await _search.Search("-filehash:cityloop -filehash-cityloop240") ).SearchCount);
		}
		...
		public async Task SearchService_SearchIOSDoubleParenthesisTreinTest()
		{
			await InsertSearchData();
			Assert.AreEqual(1,
				( await _search.Search($"“{TrashKeyword.TrashKeywordString}”") ).SearchCount);
		}
		...
		public async Task SearchService_SearchCityLoopTrimTest()
		{
			// Test TRIM
			await InsertSearchData();
			Assert.AreEqual(NumberOfFakeResults,
				( await _search.Search("   cityloop    ") ).SearchCount);
		}
		...
		public async Task SearchService_SearchCityLoopFilePathTest()
		{
			await InsertSearchData();
			Assert.AreEqual(NumberOfFakeResults,
				( await _search.Search("-FilePath:cityloop") ).SearchCount);
		}
		...
		public async Task SearchService_SearchCityloopParentDirectoryTest()
		{
			await InsertSearchData();
			Assert.AreEqual(NumberOfFakeResults,
				( await _search.Search("-ParentDirectory:/cities") ).SearchCount);
		}
		...
		public async Task SearchService_SearchForDirectories()
		{
			...
			Assert.AreEqual(1,
				( await _search.Search("-isDirectory:true -inurl:stations") ).SearchCount);
		}
		...
		public async Task SearchService_SearchNarrow2FileNameTags()
		{
			await InsertSearchData();
			Assert.AreEqual(1,
				( await _search.Search("-ParentDirectory:/station -ParentDirectory:2") )
				.SearchCount);
		}
		...
		public async Task SearchService_SearchForDateTimeBetween()
		{
			...
			Assert.AreEqual(1,
				( await _search.Search(
					"-DateTime>2015-01-01T01:01:01 -DateTime<2017-01-01T01:01:01") ).SearchCount);
		...
		public async Task SearchService_SearchAddToDatabase()
		{
			...
			Assert.AreEqual(1,
				( await _search.Search(
					"-addtodatabase>2015-01-01T01:01:01 -addtodatabase<2017-01-01T01:01:01") )
				.SearchCount);
		}
		...
		{
			var model = _search.MatchSearch(new SearchViewModel { SearchQuery = string.Empty });
				...
				{
					{
						...
						{
							FileIndexItems =
								new List<FileIndexItem> { new FileIndexItem { Tags = "t" } }
						}
					}
				});
		...
		public async Task SearchService_thisORThisFileHashes()
		{
			await InsertSearchData();
			var result =
				await _search.Search("-FileHash=stationdeletedfile || -FileHash=lelystadcentrum2",
					0, false);
		...
		{
			await InsertSearchData();
			var result = await _search.Search("-Description=lelystadcentrum2 -ImageFormat=tiff", 0,
				false);
			Assert.AreEqual(1, result.FileIndexItems?.Count);
		}
		...
		public async Task SearchService_DescriptionMultipleImageFormats()
		{
			...
			var result =
				await _search.Search(
					"-FileHash=lelystadcentrum -ImageFormat=jpg || -ImageFormat=xmp", 0, false);
		...
		{
			await InsertSearchData();
			var result = await _search.Search(
				"-FileName=lelystadcentrum.jpg || -FileHash=lelystadcentrum && lelystad",
				0, false);
		...
		public void SearchViewModel_SearchOperatorOptions_Quoted_with_ParseDefaultOption()
		{
			...
			Assert.IsTrue(searchOperatorOptions[0]);
			Assert.IsTrue(searchOperatorOptions[1]);
		}
		...
		[TestMethod]
		public void
			SearchViewModel_SearchOperatorOptions_NonQuoted_with_OR_Situation_ParseDefaultOption()
		{
			const string modelSearchQuery = "station || test";
			var searchViewModel = new SearchViewModel();
			...
			Assert.IsFalse(searchOperatorOptions[0]);
			Assert.IsFalse(searchOperatorOptions[1]);
		}

		[TestMethod]
		public void
			SearchViewModel_SearchOperatorOptions_Quoted_with_OR_Situation_ParseDefaultOption()
		{
			var modelSearchQuery = "\"station\" || \"test\"";
			var searchViewModel = new SearchViewModel();
		...
		public void SearchViewModel_TwoCharWords_ShortWord()
		{
			...
			Assert.AreEqual(SearchViewModel.SearchForOptionType.Equal,
				searchViewModel.SearchForOptions[0]);
		...
		public void SearchService_NotSingleKeywordsSearch()
		{
			var model = new SearchViewModel
			{
				...
				{
					new FileIndexItem { Tags = "lelystadcentrum" },
					new FileIndexItem { Tags = "lelystadcentrum2" },
					new FileIndexItem { Tags = "else" }
				}
			};
		...
		public void SearchService_SoftwareNull()
		{
			var model = new SearchViewModel
			{
				...
				FileIndexItems = new List<FileIndexItem>
				{
					new FileIndexItem { Tags = "software:test123" },
					new FileIndexItem { Tags = "software:test123" },
					new FileIndexItem { Tags = "lelystadcentrum", Software = "test123" }
				}
			};
