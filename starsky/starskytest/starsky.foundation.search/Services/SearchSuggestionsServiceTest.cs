...
public sealed class SearchSuggestionsServiceTest
{
	...
	public SearchSuggestionsServiceTest()
	{
		...
		_suggest = new SearchSuggestionsService(_dbContext, _memoryCache, new FakeIWebLogger(),
			new AppSettings());
		_query = new Query(_dbContext, new AppSettings(), null!, new FakeIWebLogger(),
			_memoryCache);
	}
	...
	public async Task SearchSuggestionsService_MoreThat2_StartArray()
	{
		...
		var result = await _suggest.SearchSuggest("sch", false);
	...
	public async Task SearchSuggestionsService_MoreThat2_MiddleArray()
	{
		...
		var result = await _suggest.SearchSuggest("airpl", false);
	...
	{
		var builder = new DbContextOptionsBuilder<ApplicationDbContext>();
		// this database does NOT exist! and should also not exist
		builder.UseMySql("Server=127.0.0.1;port=7544;database=test;uid=test;pwd=test;",
			new MariaDbServerVersion("10.2"));
		var options = builder.Options;
		var dbContext = new ApplicationDbContext(options);
		var fakeLogger = new FakeIWebLogger();
		var suggest =
			new SearchSuggestionsService(dbContext, _memoryCache, fakeLogger, new AppSettings());
