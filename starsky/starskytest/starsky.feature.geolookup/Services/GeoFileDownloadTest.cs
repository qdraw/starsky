...
public class GeoFileDownloadTests
{
	...
	public GeoFileDownloadTests()
	{
		...
		_dependenciesFolder1 =
			Path.Combine(new CreateAnImage().BasePath, "GeoFileDownloadTests-deps_01");
		_dependenciesFolder2 =
			Path.Combine(new CreateAnImage().BasePath, "GeoFileDownloadTests-deps_02");
		_dependenciesFolder3 =
			Path.Combine(new CreateAnImage().BasePath, "GeoFileDownloadTests-deps_03");
		_dependenciesFolder4 =
			Path.Combine(new CreateAnImage().BasePath, "GeoFileDownloadTests-deps_04");
		_dependenciesFolder5 =
			Path.Combine(new CreateAnImage().BasePath, "GeoFileDownloadTests-deps_05");
		_dependenciesFolder6 =
			Path.Combine(new CreateAnImage().BasePath, "GeoFileDownloadTests-deps_06");
	}
	...
	public async Task RemoveFailDownload_FileToSmall_SoRemove()
	{
		...
		Assert.IsTrue(storage.ExistFile(Path.Combine(_dependenciesFolder5,
			GeoFileDownload.CountryName + ".zip")));

		geoFileDownload.RemoveFailedDownload();

		Assert.IsTrue(storage.ExistFolder(_dependenciesFolder5));
		Assert.IsFalse(storage.ExistFile(Path.Combine(_dependenciesFolder5,
			GeoFileDownload.CountryName + ".zip")));
	}
	...
	public async Task RemoveFailDownload_FileRightSize_SoKeep()
	{
		...
		Assert.IsTrue(storage.ExistFile(Path.Combine(_dependenciesFolder6,
			GeoFileDownload.CountryName + ".zip")));
