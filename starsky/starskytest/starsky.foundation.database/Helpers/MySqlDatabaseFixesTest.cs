...
public sealed class MySqlDatabaseFixesTest
{
	...
	{
		var result = await new MySqlDatabaseFixes(null,
			new AppSettings { DatabaseType = AppSettings.DatabaseTypeList.InMemoryDatabase },
	...
	public async Task OpenConnection_MySqlConnector_MySqlException()
	{
		...
		await new MySqlDatabaseFixes(new MySqlConnection(fakeConnectionString),
				new AppSettings { DatabaseType = AppSettings.DatabaseTypeList.Mysql }, logger)
			.OpenConnection();

		Assert.IsTrue(
			logger.TrackedExceptions.Exists(x => x.Item2?.Contains("MySqlException") == true));
	}
	...
	{
		var result = await new MySqlDatabaseFixes(new MySqlConnection(),
			new AppSettings { DatabaseType = AppSettings.DatabaseTypeList.InMemoryDatabase },
			new FakeIWebLogger()).FixAutoIncrement("test");
		Assert.IsNull(result);
	}
	...
	public async Task CheckAutoIncrementExist_DueMissingDatabase()
	{
		var appSettings = new AppSettings
		{
			...
			DatabaseConnection =
				"Server=localhost;port=4785;database=TEST;uid=starsky;pwd=TEST;maximumpoolsize=30;"
		};
		var mysqlConnection = new MySqlConnection(appSettings.DatabaseConnection);
		var result =
			await new MySqlDatabaseFixes(mysqlConnection, appSettings, new FakeIWebLogger())
				.CheckAutoIncrementExist("test");
		Assert.IsFalse(result);
	}
	...
	{
		var appSettings = new AppSettings { DatabaseType = AppSettings.DatabaseTypeList.Mysql };
		var result = await new MySqlDatabaseFixes(null, appSettings, new FakeIWebLogger())
			.CheckAutoIncrementExist("test");
		Assert.IsNull(result);
	}
