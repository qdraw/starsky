...
public sealed class OnStartupSyncBackgroundServiceTest
{
	...
	public async Task OnStartupSyncBackgroundService_DoesStoreAfterWards()
	{
		...
		{
			setting = await settingsService.GetSetting<DateTime>(SettingsType
		...
		Assert.IsGreaterThanOrEqualTo(2000, setting.Year,
			"LastSyncBackgroundDateTime was not written in time");
		var delta = ( DateTime.UtcNow - setting.ToUniversalTime() ).Duration();
		Assert.IsLessThan(TimeSpan.FromMinutes(2), delta,
			$"LastSyncBackgroundDateTime is not recent: delta={delta}");
