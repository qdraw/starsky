...
public sealed class ExifToolDownloadTest
{
	...
	public static void CleanExifToolDownloadTest()
	{
		...
		host.FolderDelete(Path.Combine(new CreateAnImage().BasePath,
			"starsky-tmp-dependencies-4835793"));
	}
	...
	public async Task DownloadExifTool_existVerbose_UnixOnly()
	{
		...
		RemoveTempFolderWithExifTool("test32");
	}
	...
	public async Task StartDownloadForWindows_WrongHash()
	{
		...
		Assert.IsFalse(result);
		Assert.IsTrue(
			logger.TrackedExceptions.Exists(p => p.Item2?.Contains("Checksum for ") == true));
	}
	...
	public async Task DownloadForUnix_FromMirrorInsteadOfMainSource()
	{
		...
		Assert.IsTrue(
			logger.TrackedExceptions.Exists(p => p.Item2?.Contains("Checksum for ") == true));
		Assert.IsTrue(
			logger.TrackedExceptions.Exists(p => p.Item2?.Contains("is not valid") == true));
	}
	...
	[TestMethod]
	public async Task StartDownloadForWindows_MirrorChecksumFailed_EmthyResult()
	{
		var fakeIHttpProvider = new FakeIHttpProvider(new Dictionary<string, HttpContent>
		{
			{ "https://exiftool.org/checksums.txt", new StringContent(ExampleCheckSum) },
