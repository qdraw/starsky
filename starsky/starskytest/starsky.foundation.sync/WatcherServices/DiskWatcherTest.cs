...
public sealed class DiskWatcherTest
{
	...
	public DiskWatcherTest()
	{
		...
		services
			.AddScoped<IDiskWatcherBackgroundTaskQueue, FakeDiskWatcherUpdateBackgroundTaskQueue>();
	...
	public void DiskWatcherTest_Watcher_Error()
	{
		...
		var synchronize =
			scope.ServiceProvider.GetRequiredService<ISynchronize>() as FakeISynchronize;
		if ( synchronize == null )
		{
		...
		fakeIFileSystemWatcher.TriggerOnError(
			new ErrorEventArgs(new InternalBufferOverflowException("test")));
		fakeIFileSystemWatcher.TriggerOnError(
			new ErrorEventArgs(new InternalBufferOverflowException("test")));
	...
	public void Watcher_Changed()
	{
		...
		fakeIFileSystemWatcher.TriggerOnChanged(new FileSystemEventArgs(WatcherChangeTypes.Changed,
			"/", "test.jpg"));
	...
	public void Watcher_CrashAnd_Retry()
	{
		var fakeIFileSystemWatcher = new FakeIFileSystemWatcherWrapper
		{
			CrashOnEnableRaisingEvents = true, Path = new CreateAnImage().BasePath
		};
	...
	public void OnRenamed_Should_Not_HitQueueProcessor_FromTmp2()
	{
		...
		var event1 = new RenamedEventArgs(WatcherChangeTypes.Renamed,
			_createAnImage.BasePath, _createAnImage.FileName,
			Path.DirectorySeparatorChar + "tmp.{guid}.test.jpg");
