...
public class DeleteItem(IQuery query, AppSettings appSettings, ISelectorStorage selectorStorage)
	: IDeleteItem
{
	private readonly IStorage _iStorage =
		selectorStorage.Get(SelectorStorage.StorageServices.SubPath);

	private readonly StatusCodesHelper _statusCodeHelper = new(appSettings);
	...
	public async Task<List<FileIndexItem>> DeleteAsync(string filePath, bool collections)
	{
		...
		{
			var detailView = query.SingleItem(subPath, null, collections, false);
			...
			var items =
				( await query.GetAllFilesAsync(detailView.FileIndexItem.FilePath) )
				.Select(itemInDirectory => itemInDirectory.FilePath!);
		...
		List<FileIndexItem> fileIndexResultsList)
	{
		...
		{
			var detailViewItem = query.SingleItem(collectionSubPath, null, false, false);
			...
			// remove item from db
			await query.RemoveItemAsync(detailViewItem.FileIndexItem);
			...
			foreach ( var item in
			         ( await query.GetAllRecursiveAsync(collectionSubPath) ).Where(p =>
				         p.IsDirectory == true) )
			{
