...
public class PreviewImageExtractor
{
	...
	private void ReadExifTags(BinaryReader reader, int length)
	{
		...
		while ( reader.BaseStream.Position < endPosition )
		{
			...
			var componentCount = ReadBigEndianUInt32(reader);
			var dataOffset = ReadBigEndianUInt32(reader);
			...
			var data = reader.ReadBytes(componentCount * 2);

			// Display tag data
			DisplayTagData(data, format);
			reader.BaseStream.Seek(currentPosition,
				SeekOrigin.Begin); // Return to original position
