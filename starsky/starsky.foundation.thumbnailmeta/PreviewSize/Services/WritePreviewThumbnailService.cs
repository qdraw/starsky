...
using starsky.foundation.thumbnailmeta.PreviewSize.Helpers;
using starsky.foundation.thumbnailmeta.PreviewSize.Interfaces;

namespace starsky.foundation.thumbnailmeta.PreviewSize.Services;
...
public class WritePreviewThumbnailService : IWritePreviewThumbnailService
{
	...
	public async Task<bool> WriteFile(string fileHash, OffsetModel offsetData,
		ImageRotation.Rotation rotation,
		string? reference = null)
	{
		...
		var thumbnailFromThumbnailUpdateList =
			new ThumbnailSizesChecker(_thumbnailStorage, _appSettings).ListThumbnailToBeCreated(
	...
	private async Task<bool> ResizeAsync(OffsetModel offsetData, ImageRotation.Rotation rotation,
		ThumbnailSize size, string fileHash, string? reference)
	{
		...
		try
		{
			...
			{
				previewImage.Mutate(i => i.Resize(size.Width(), 0, KnownResamplers.Lanczos3));
				previewImage.Mutate(i =>
					i.Rotate(rotation.ToDegrees()));

				await previewImage.SaveAsJpegAsync(outputStream);

				await _thumbnailStorage.WriteStreamAsync(outputStream,
					ThumbnailNameHelper.Combine(fileHash, size, ThumbnailImageFormat.jpg));
