...
public sealed class Synchronize : ISynchronize
{
		...
		IMemoryCache? memoryCache = null)
	{
		...
		_syncSingleFile =
			new SyncSingleFile(appSettings, query, _subPathStorage, memoryCache, logger);
		...
		_syncFolder = new SyncFolder(appSettings, query, selectorStorage, _console, logger,
			memoryCache, serviceScopeFactory);
		_syncIgnoreCheck = new SyncIgnoreCheck(appSettings, _console);
		_syncMultiFile =
			new SyncMultiFile(appSettings, query, _subPathStorage, memoryCache, logger);
	}
