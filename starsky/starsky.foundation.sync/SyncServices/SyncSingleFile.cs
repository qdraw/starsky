...
public sealed class SyncSingleFile
{
		...
		IMemoryCache? memoryCache, IWebLogger logger)
	{
		...
		_query = query;
		_checkForStatusNotOkHelper = new CheckForStatusNotOkHelper(subPathStorage, logger);
		_logger = logger;
		...
		ISynchronize.SocketUpdateDelegate? updateDelegate = null)
	{
		...
		foreach ( var item in dbItems.Where(item =>
			         scanItems.TrueForAll(p => p.FilePath != item.FilePath)) )
		{
			scanItems.Add(item);
		}
	...
	public async Task UpdateSidecarFile(string xmpSubPath)
	{
		...
		var directoryWithFileIndexItems = ( await
			_query.GetAllFilesAsync(parentPath) ).Where(p => p.ParentDirectory == parentPath &&
			                                                 p.FileCollectionName ==
			                                                 fileNameWithoutExtension).ToList();
	...
	/// <returns>completed task</returns>
	internal async Task<bool> UpdateSidecarFile(string xmpSubPath,
		List<FileIndexItem> directoryWithFileIndexItems)
	{
		if ( !ExtensionRolesHelper.IsExtensionSidecar(xmpSubPath) )
		{
			return false;
		...
		foreach ( var item in
		         directoryWithFileIndexItems.Where(item =>
			         !item.SidecarExtensionsList.Contains(sidecarExt)) )
		{
