...
/// <summary>
///     @see:
///     http://web.archive.org/web/20210415105229/https://petermeinl.wordpress.com/2015/05/18/tamed-filesystemwatcher/
/// </summary>
...
public sealed class BufferingFileSystemWatcher : Component, IFileSystemWatcherWrapper
{
	...
	private BlockingCollection<FileSystemEventArgs> _fileSystemEventBuffer = new();
	...
	private void RaiseBufferedEventsUntilCancelled()
	{
		Task.Run(() =>
		{
			try
			{
				...
				foreach ( var fileSystemEventArgs in
	...
	internal void NotifyExistingFiles()
	{
		...
		{
			var sortedFileInfos =
				from fi in new DirectoryInfo(Path).GetFiles(Filter, searchSubDirectoriesOption)
			...
			foreach ( var fi in sortedFileInfos )
			{
				...
				InvokeHandler(_onExistedHandler,
					new FileSystemEventArgs(WatcherChangeTypes.All, fi.DirectoryName, fi.Name));
		...
		{
			foreach ( var fsi in new DirectoryInfo(Path).EnumerateFileSystemInfos(Filter,
				         searchSubDirectoriesOption) )
			{
				var directoryName =
				...
				InvokeHandler(_onExistedHandler,
					new FileSystemEventArgs(WatcherChangeTypes.All, directoryName, fsi.Name));
				InvokeHandler(_onAllChangesHandler,
					new FileSystemEventArgs(WatcherChangeTypes.All, directoryName, fsi.Name));
			}
		}
