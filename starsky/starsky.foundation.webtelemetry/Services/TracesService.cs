...
public class TracesService: ITracesService
{
	...
	private List<KeyValuePair<string, string>> ParseHeader()
	{
		...
		foreach (var header in headerArray)
		{
			...
			{
				metadata.Add(new KeyValuePair<string, string>(keyValue[0].Trim(), keyValue[1].Trim()));
			}
		}
		return metadata;
	}
	...
	public async Task SendTrace(TracesData tracesData)
	{
		...
		var response = await _httpProvider.PostAsync(_appSettings.OpenTelemetry.TracesEndpoint, content);
