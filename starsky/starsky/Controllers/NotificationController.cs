...
[Authorize]
public sealed class NotificationController(INotificationQuery notificationQuery) : Controller
{
	/// <summary>
	...
	public async Task<IActionResult> GetNotifications(string dateTime)
	{
		...
		return Json(await notificationQuery.GetNewerThan(parsedDateTime.ToUniversalTime()));
	}

	internal static Tuple<bool, DateTime> ParseDate(string dateTime)
	{
		var isParsed = DateTime.TryParse(dateTime, CultureInfo.InvariantCulture,
			DateTimeStyles.AssumeUniversal, out var parsedDateTime);
		if ( isParsed )
		{
