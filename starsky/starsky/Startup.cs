...
public sealed class Startup
{
		...
		services.AddAntiforgery(
			options =>
			{
				...
				options.Cookie.HttpOnly =
					true; // only used by .NET, there is a separate method to generate an X-XSRF-TOKEN cookie
	...
	///     test without: `curl http://localhost:4000/starsky/manifest.json --silent --write-out
	///     "%{size_download}\n" --output /dev/null`
