...
export async function ChangeSettingMappings(
  mappings: MappingRow[]
): Promise<number> {
...
const PreferencesAppSettingsStorageFolderMappings: React.FunctionComponent =
  () => {
    ...
    const MessageChangeNeedReSync = language.key(
      localization.MessageChangeNeedReSync
    );
    ...
    useEffect(() => {
      ...
      setIsEnabled(
        data.includes(new UrlQuery().KeyAccountPermissionAppSettingsWrite())
      );
    }, [permissionsData]);
    ...
    function handleSubPathBlur(id: number, value: string) {
      const updated = mappings.map((r) =>
        r.id === id ? { ...r, subPath: value } : r
      );
      setMappings(updated);
      saveAll(updated);
    }
    ...
    function handlePhysicalPathBlur(id: number, value: string) {
      const updated = mappings.map((r) =>
        r.id === id ? { ...r, physicalPath: value } : r
      );
      setMappings(updated);
      saveAll(updated);
    }
    ...
    function addRow() {
      setMappings((prev) => [
        ...prev,
        { id: nextId.current++, subPath: "", physicalPath: "" }
      ]);
    }
