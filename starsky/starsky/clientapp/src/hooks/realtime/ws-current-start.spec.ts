...
import WsCurrentStart, { FireOnClose, FireOnError, FireOnMessage, FireOnOpen, HandleKeepAliveMessage } from './ws-current-start';
...
describe("WsCurrentStart", () => {
  ...
  describe("WsCurrentStart or default", () => {
    it("is connected and returns default", () => {
      ...
      var wsCurrentStart = WsCurrentStart(true, setSocketConnectedSpy, { current: true },
        jest.fn(), NewFakeWebSocketService) as FakeWebSocketService;
  ...
  describe("FireOnClose", () => {
    ...
    it("check if setSocketConnected is called", () => {
      var setSocketConnectedSpy = jest.fn()
      FireOnClose(new CloseEvent("t", { code: 1000 }), true, setSocketConnectedSpy, { current: true });
      expect(setSocketConnectedSpy).toBeCalled();
    });
  });
  ...
  describe("FireOnMessage", () => {
    it("check if setKeepAliveTimeSpy is on Welcome", () => {
      ...
      FireOnMessage(new MessageEvent("t", { data: "{\"welcome\": true}" }), setKeepAliveTimeSpy);
      expect(setKeepAliveTimeSpy).toBeCalled();
    });

    it("check if setKeepAliveTimeSpy is on Time", () => {
      var setKeepAliveTimeSpy = jest.fn()
      FireOnMessage(new MessageEvent("t", { data: "{\"time\": 1}" }), setKeepAliveTimeSpy);
      expect(setKeepAliveTimeSpy).toBeCalled();
    });
