...
public static class EnumerableExtensions
{
		...
		int maxDegreesOfParallelism = 6)
	{
		var transformBlock = new TransformBlock<TSource, TResult>(action,
			new ExecutionDataflowBlockOptions { MaxDegreeOfParallelism = maxDegreesOfParallelism });
	...
	public static T? FirstOrDefaultWithFallback<T>(
		...
		switch ( value )
		{
			...
			default:
			{
				...
				return collection.Count <= 1
					...
					: collection.FirstOrDefault(p => EqualityComparer<T>.Default.Equals(p, value));
			}
		}
	}
}
