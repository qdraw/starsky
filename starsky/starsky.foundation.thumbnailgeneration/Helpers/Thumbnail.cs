...
public sealed class Thumbnail
{
	...
	/// <summary>
	///     Create a Thumbnail file to load it faster in the UI. Use FileIndexItem or database style path,
		...
		string fileHash, bool skipExtraLarge = false)
	{
		...
		var thumbnailFromThumbnailUpdateList =
			new ThumbnailSizesChecker(_thumbnailStorage).ListThumbnailToBeCreated(
				[.. ThumbnailNameHelper.SecondGeneratedThumbnailSizes],
				fileHash);
		var largeImageResult = await CreateLargestImageFromSource(fileHash, largeThumbnailHash,
			subPath, thumbnailToSourceSize);
	...
	public async Task<(MemoryStream?, GenerationResultModel)> ResizeThumbnailFromThumbnailImage(
	...
	)
	{
		...
		catch ( Exception ex )
		{
			...
			_logger.LogError($"[ResizeThumbnailFromThumbnailImage] Exception {fileHash} {message}",
				ex);
