...
public class GeoIndexGpx : IGeoIndexGpx
{
	...
	public async Task<List<FileIndexItem>> LoopFolderAsync(List<FileIndexItem> metaFilesInDirectory)
	{
			         ...
			         new { value, index }) )
		{
			...
			var fileGeoData =
				gpxList.MinBy(p => Math.Abs(( p.DateTime - dateTimeCameraUtc ).Ticks));
			if ( fileGeoData == null )
			{
				continue;
			}
	...
	private static List<FileIndexItem> GetNoLocationItems(
		IEnumerable<FileIndexItem> metaFilesInDirectory)
	{
		return metaFilesInDirectory.Where(metaFileItem =>
				Math.Abs(metaFileItem.Latitude) < 0.001 && Math.Abs(metaFileItem.Longitude) < 0.001
				                                        && metaFileItem.DateTime.Year >
				                                        2) // ignore files without a date
			.ToList();
	}
	...
	internal DateTime ConvertTimeZone(DateTime valueDateTime, string subPath = "")
	{
		     ...
		     _appSettings.CameraTimeZoneInfo == null )
		{
			throw new ArgumentException($"valueDateTime DateTime-Kind '{valueDateTime.Kind}' " +
			                            $"'{subPath}' should be Unspecified",
				nameof(valueDateTime));
		}
