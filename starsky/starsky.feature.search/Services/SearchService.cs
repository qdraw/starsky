...
public class SearchService : ISearch
{
	...
	private static SearchViewModel SkipSearchItems(object? objectSearchModel, int pageNumber)
	{
		...
		var skipLastNumber = searchModel.SearchCount - pageNumber * NumberOfResultsInView -
		                     NumberOfResultsInView;
		...
		for ( var i = 0; i < model.SearchIn.Count; i++ )
		{
			...
			switch ( searchInType )
			{
				...
				case SearchViewModel.SearchInTypes.filehash:
					...
					predicates.Add(x =>
						x.FileHash != null && x.FileHash.ToLower().Contains(fileHash));
					break;
				...
				case SearchViewModel.SearchInTypes.lastedited:
					predicates.Add(SearchWideDateTime.WideSearchDateTimeGet(model, i,
						SearchWideDateTime.WideSearchDateTimeGetType.LastEdited));
					break;
				case SearchViewModel.SearchInTypes.addtodatabase:
					predicates.Add(SearchWideDateTime.WideSearchDateTimeGet(model, i,
						SearchWideDateTime.WideSearchDateTimeGetType.AddToDatabase));
					break;
				case SearchViewModel.SearchInTypes.datetime:
					predicates.Add(SearchWideDateTime.WideSearchDateTimeGet(model, i,
						SearchWideDateTime.WideSearchDateTimeGetType.DateTime));
					break;
