...
public class SearchViewModel
{
	...
	public List<string> SearchFor
	{
		...
		get { return SearchForInternal ?? new List<string>(); }
	}
	...
	{
		get { return SearchForOptionsInternal ?? new List<SearchForOptionType>(); }
	}
	...
	public string PageType
	{
		get
		{
			...
			return SearchQuery == TrashKeyword.TrashKeywordString
				? PageViewType.PageType.Trash.ToString()
	...
	public double ElapsedSeconds
	{
		...
		set { _elapsedSeconds = value - value % 0.001; }
	}
	...
	{
		get { return SearchOperatorOptionsInternal ?? new List<bool>(); }
	}
	...
	public void SetAndOrOperator(char andOrChar, int relativeLocation = 0)
	{
		...
		{
			SearchOperatorOptionsInternal.Insert(
				SearchOperatorOptionsInternal.Count + relativeLocation, andOrBool);
		}
	}
		...
		ParseQuotedValues(string defaultQuery, StringBuilder returnQueryBuilder)
	{
		...
		foreach ( Match regexInUrl in regexInUrlMatches )
		{
			...
			if ( ( regexInUrl.Index - 1 >= 0 && defaultQuery[regexInUrl.Index - 1] == '-' )
			     || ( regexInUrl.Index + 2 <= regexInUrl.Length &&
		...
		SearchForOptionType searchType)
	{
		switch ( searchType )
		{
			case SearchForOptionType.Not:
				model.FileIndexItems = model.FileIndexItems!.Where(p =>
					p.GetType().GetProperty(property.Name)?.Name == property.Name
					&& ! // not
		...
		PropertyInfo? property, bool boolIsValue)
	{
		...
		model.FileIndexItems = model.FileIndexItems?
			.Where(p => p.GetType().GetProperty(property.Name)?.Name == property.Name
			            && ( bool? ) p.GetType().GetProperty(property.Name)?.GetValue(p, null) ==
			            boolIsValue
			).ToList();
		return model;
	}
		...
		SearchForOptionType searchType)
	{
		switch ( searchType )
		{
			case SearchForOptionType.Not:
				model.FileIndexItems = model.FileIndexItems!
					.Where(p => p.GetType().GetProperty(property.Name)?.Name == property.Name
					            && ( ExtensionRolesHelper.ImageFormat ) p.GetType()
		...
		SearchForOptionType searchType)
	{
		...
		switch ( searchType )
		{
			case SearchForOptionType.LessThen:
				model.FileIndexItems = model.FileIndexItems!
					.Where(p => p.GetType().GetProperty(property.Name)?.Name == property.Name
					            && ( DateTime ) p.GetType().GetProperty(property.Name)
						            ?.GetValue(p, null)!
					            <= parsedDateTime
					).ToList();
				break;
