...
public static class ServiceCollectionExtensions
{
		...
		params Assembly[] assemblies)
	{
		var typesWithAttributes = assemblies
			...
			.Select(type => new
			{
				Lifetime = type?.GetCustomAttribute<ServiceAttribute>()?.InjectionLifetime,
				ServiceType = type,
		...
		foreach ( var type in typesWithAttributes )
		{
			...
			{
				serviceCollection.Add(type.ImplementationType, type.ServiceType!,
					type.Lifetime!.Value);
			}
		}
	}

	public static void Add(this IServiceCollection serviceCollection, InjectionLifetime ioCLifetime,
		params Type[] types)
	{
		if ( types == null )
		{
	...
	public static void Add<T>(this IServiceCollection serviceCollection,
		InjectionLifetime ioCLifetime)
	{
		serviceCollection.Add(typeof(T), ioCLifetime);
	}

	private static void Add(this IServiceCollection serviceCollection, Type type,
		InjectionLifetime ioCLifetime)
	{
		switch ( ioCLifetime )
		{
	...
	private static List<Assembly> GetEntryAssemblyReferencedAssemblies(List<Assembly> assemblies,
	...
	private static Assembly[] GetReferencedAssemblies(List<Assembly> assemblies,
