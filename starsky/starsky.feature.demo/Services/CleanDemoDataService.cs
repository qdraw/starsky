...
public sealed class CleanDemoDataService(IServiceScopeFactory serviceScopeFactory)
	: BackgroundService
{
	...
	public async Task<bool?> RunAsync()
	{
		...
		if ( appSettings.DemoUnsafeDeleteStorageFolder != true || appSettings.ApplicationType !=
	...
	internal static PublishManifestDemo? Deserialize(string result, IWebLogger webLogger,
		IStorage hostStorage, string settingsJsonFullPath)
	{
		...
		IStorage subStorage, IWebLogger webLogger)
	{
		...
		foreach ( var (jsonUrl, dir) in appSettings.DemoData )
		{
			...
			if ( !hostStorage.ExistFile(settingsJsonFullPath) &&
			     !await httpClientHelper.Download(jsonUrl, settingsJsonFullPath) )
			{
