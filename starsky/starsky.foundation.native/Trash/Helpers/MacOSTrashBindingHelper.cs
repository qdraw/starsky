...
public static class MacOsTrashBindingHelper
{
	...
	internal static IntPtr[] GetUrls(List<string> filesFullPath)
	{
		...
		foreach ( var filePath in filesFullPath )
		{
			...
			var fileUrl =
				objc_msgSend_retIntPtr_IntPtr(nsUrl, GetSelector("fileURLWithPath:"),
					cfStrTestFile);
	...
	[DllImport(FoundationFramework)]
	private static extern IntPtr CFArrayCreate(IntPtr allocator, IntPtr values, long numValues,
		IntPtr callbackStruct);
	...
	[DllImport(FoundationFramework, EntryPoint = "objc_msgSend")]
	private static extern void objc_msgSend_retVoid_IntPtr_IntPtr(IntPtr target, IntPtr selector,
